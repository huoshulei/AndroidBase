package com.example.icogn.mshb.frame.utils;
...
public final class App {
    public static final MyApplication application;

    static {
        MyApplication app = null;
        try {
            app = (MyApplication) Class.forName("android.app.AppGlobals").getMethod("getInitialApplication").invoke(null);
            if (app == null)
                throw new IllegalStateException("Static initialization of Applications must be on main thread.");
        } catch (Exception e) {
            Logger.d("Failed to get current application from AppGlobals." + e.getMessage());
            try {
                app = (MyApplication) Class.forName("android.app.ActivityThread").getMethod("currentApplication").invoke(null);
            } catch (Exception e1) {
